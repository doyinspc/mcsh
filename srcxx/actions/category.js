...
export const updateCategory = (id, data) => (dispatch, getState) => {
    ...
    axios.patch(paths, body, categorySetConfig1(getState))
        .then(res => {
            dispatch({
...
export const categorySetConfig = getState => {
    //get token from state
    const token = getState().categoryReducer.token;
...
//SET TOKEN AND HEADER - HELPER FUNCTION
export const categorySetConfig1 = getState => {
    //get token from state
    const token = getState().categoryReducer.token;
